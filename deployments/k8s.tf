...
resource "kubernetes_namespace" "bpm_cli_namespace" {
  metadata {
...
resource "kubernetes_limit_range" "bpm_cli_namespace_limits" {
  metadata {
...
resource "kubernetes_secret" "bpm_cli_image_pull_secret" {
  metadata {
...
resource "kubernetes_service" "bpm_cli_svc" {
  metadata {
    namespace = var.k8s_namespace
    name      = "bpm-cli-docs-svc"
    labels    = local.common_labels
  }
  spec {
...
resource "kubernetes_pod_disruption_budget" "bpm_cli_pdb" {
  metadata {
    namespace = var.k8s_namespace
    name      = "bpm-cli-docs-pdb"
    labels    = local.common_labels
  }
  spec {
    selector {
...
resource "kubernetes_ingress" "bpm_cli_ingress" {
  metadata {
    namespace = var.k8s_namespace
    name      = "bpm-cli-docs-ingress"
    labels    = local.common_labels
    ...
    rule {
      ...
      http {
        path {
          path = "/"
          backend {
            service_name = kubernetes_service.bpm_cli_svc.metadata[0].name
            service_port = "80"
          }
        }
      }
